        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form id="delete_{{ $image->id }}" action="{{ route('owner.images.destroy', ['image' => $image->id]) }}" method="post" class="lg:w-2/3 mx-auto pt-6 flex justify-end">
                        @method('DELETE')
                        @csrf
                        <a href="#" data-id="{{ $image->id }}" onclick="deletePost(this)" class="flex text-white bg-red-400 border-0 py-2 px-4 focus:outline-none hover:bg-red-500 rounded">削除</a>
                    </form>
                </div>
            </div>
        </div>
